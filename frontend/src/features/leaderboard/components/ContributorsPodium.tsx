...
export function ContributorsPodium({ topThree, isLoaded, actualCount }: ContributorsPodiumProps) {
  ...
  return (
    <div className="flex items-end justify-center gap-4 mt-8">
      {/* 2nd Place */}
      {showSecond && (
        <div className={`flex flex-col items-center transition-all duration-700 delay-700 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
          }`}>
          <div className="backdrop-blur-[30px] bg-gradient-to-br from-white/[0.25] to-white/[0.15] border-2 border-white/40 rounded-[18px] p-6 w-[150px] shadow-[0_6px_24px_rgba(0,0,0,0.1)] mb-3 hover:shadow-[0_8px_28px_rgba(0,0,0,0.15)] hover:scale-105 transition-all duration-300 group">
            ...
            <div className="text-center">
              <div className={`text-[14px] font-bold mb-1 transition-colors ${theme === 'dark' ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
                }`}>
                {topThree[1].first_name || topThree[1].last_name
                  ? `${topThree[1].first_name || ''} ${topThree[1].last_name || ''}`.trim()
                  : topThree[1].username}
              </div>
          ...
          <div className="flex items-center justify-center gap-1.5 backdrop-blur-[20px] bg-white/[0.2] border border-white/30 rounded-[10px] px-3 py-1.5 shadow-sm animate-slide-up-delayed">
            ...
            <span className={`text-[16px] font-bold transition-colors ${theme === 'dark' ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
              }`}>#2</span>
          </div>
        </div>
      )}
      ...
      {/* 1st Place */}
      <div className={`flex flex-col items-center -mt-8 transition-all duration-700 delay-600 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
        }`}>
        <div className="relative backdrop-blur-[30px] bg-gradient-to-br from-[#c9983a]/30 to-[#d4af37]/20 border-2 border-[#c9983a]/60 rounded-[20px] p-7 w-[170px] shadow-[0_8px_32px_rgba(201,152,58,0.35)] mb-3 hover:shadow-[0_12px_40px_rgba(201,152,58,0.5)] hover:scale-110 transition-all duration-300 group">
          ...
          <div className="relative">
            ...
            <div className="text-center">
              <div className={`text-[15px] font-bold mb-1 transition-colors ${theme === 'dark' ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
                }`}>
                {topThree[0].first_name || topThree[0].last_name
                  ? `${topThree[0].first_name || ''} ${topThree[0].last_name || ''}`.trim()
                  : topThree[0].username}
              </div>
        ...
        <div className="flex items-center justify-center gap-1.5 backdrop-blur-[20px] bg-gradient-to-br from-[#c9983a]/40 to-[#d4af37]/30 border-2 border-[#c9983a]/70 rounded-[12px] px-4 py-2 shadow-md animate-slide-up">
          ...
          <span className={`text-[18px] font-bold transition-colors ${theme === 'dark' ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
            }`}>#1</span>
        </div>
      </div>
      ...
      {/* 3rd Place */}
      {showThird && (
        <div className={`flex flex-col items-center transition-all duration-700 delay-800 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'
          }`}>
          <div className="backdrop-blur-[30px] bg-gradient-to-br from-white/[0.25] to-white/[0.15] border-2 border-white/40 rounded-[18px] p-6 w-[150px] shadow-[0_6px_24px_rgba(0,0,0,0.1)] mb-3 hover:shadow-[0_8px_28px_rgba(0,0,0,0.15)] hover:scale-105 transition-all duration-300 group">
            <div className="relative">
            ...
            <div className="text-center">
              <div className={`text-[14px] font-bold mb-1 transition-colors ${theme === 'dark' ? 'text-[#f5f5f5]' : 'text-[#2d2820]'
                ...
                {topThree[2].first_name || topThree[2].last_name
                  ? `${topThree[2].first_name || ''} ${topThree[2].last_name || ''}`.trim()
                  : topThree[2].username}
              </div>
